from aiohttp import web  # 部署web服务器
from datetime import datetime
from jinja2 import Environment, FileSystemLoader
import asyncio, os, json, time  # asyncio异步IO支持库
import logging
logging.basicConfig(level=logging.INFO)  # 日志-级别INFO
...
def init_jinja2(app, **kw):
    ...
    # Enviroment这个类的实例用于存储配置和全局对象，然后从文件系统或其他位置中加载模板
    env = Environment(loader=FileSystemLoader(path),
                      **options)  # FileSystemLoader：文件系统加载器,可以直接访问系统中的文件
    filters = kw.get('filters', None)  # filter过滤器可以理解为是jinja2里面的内置函数和字符串处理函数
...
# 需要handlers.py的支持, 当handlers.py在API章节里完全编辑完再将下面代码的井号去掉
