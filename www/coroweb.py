...
# 编写装饰函数@get(),在代码运行期间动态增加功能的方式叫装饰器,本质上,decorator就是一个返回函数的高阶函数
def get(path):
    #Define decorator @get('/path')
    def decorator(func):
        @functools.wraps(func)  #包裹func函数保证函数不改变
        def wrapper(*args, **kw):
            return func(*args, **kw)
...
"""
inspect模块帮助获取关于活动对象的信息，例如模块、类、方法、函数、回溯、框架对象和代码对象
signature函数的签名信息：它包含了了函数的函数名、它的参数类型，它所在的类和名称空间及其他信息，签名对象(signature object)表示可调用对象的调用签名信息和它的注解信息，signature()返回一个包含可调用对象信息的签名对象
    ...
    async def __call__(self, request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            if request.method == 'POST':
                if not request.content_type:  #在Http协议消息头中，使用Content-Type来表示具体请求中的媒体类型信息。
                    return web.HTTPBadRequest(text='Missing Content-Type.')
                ct = request.content_type.lower()
                if ct.startswith(
                        'application/json'
                ):  # startswith() 方法用于检查字符串是否是以指定子字符串开头，如果是则返回 True，否则返回 False，此处使用'application/json'作为content_type响应头
                    ...
                    kw = params
                        ...
                        'application/x-www-form-urlencoded') or ct.startswith(
                            'multipart/form-data'):  #常见的 POST 提交数据的方式
                    params = await request.post()
                    kw = dict(**params)
                else:
                    return web.HTTPBadRequest(
                        text='Unsupported Content-Type: %s' %
                        request.content_type)
            if request.method == 'GET':
                qs = request.query_string  #得到url中？后面所有的值，为一个字符串，如：wzd=111&abc=cc
                if qs:
                    kw = dict()
...
#定义add_static函数来注册static文件夹下的文件
def add_static(app):
...
#定义add_routes函数，自动把handler模块所有符合条件的URL函数注册了
